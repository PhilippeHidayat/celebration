...
exports.getTimezoneByIp = getTimezoneByIp;
...
async function getTimezoneByIp(ip) {
    try {
        ...
        const data = (await response.json()); // Type the data
        if (data && data.timezone) {
            // Convert UTC offset to GMT format
            const offset = polyfill_1.Temporal.Now.instant().toZonedDateTimeISO(data.timezone).offset; // e.g., "+07:00"
